...
Once a line has been transcribed, it will persist on the image, meaning that other volunteers will see it when they encounter that Subject. The transcription can then be reviewed by other volunteers, who will have the option to either select it from a dropdown menu—indicating that they agree with the initial transcription—or input a new transcription either by selecting and editing the previous transcription or typing an entirely new one. 

Retirement for workflows using the Transcription Task is consensus-based, which is different than most Zooniverse workflows, which use count-based retirement. Once the text has been agreed upon by the required number of volunteers (see Retirement Limits in section 6) the drawn line will turn gray, indicating it has reached consensus and no longer needs additional review. No additional transcriptions may be submitted for a line once it has turned gray. The default consensus setting for this task is 3, meaning that a line will turn gray once 3 identical transcriptions have been submitted. This may mean that your total number of transcriptions submitted ends up being higher (e.g. 5 or 6) but that is because a few people submitted transcriptions that did not match the consensus version. The Transcription Task also includes a backup, count-based retirement method. If 30 different people transcribe on a given image without reaching consensus, the image will be retired. This helps ensure that images that are unlikely to achieve consensus transcription (e.g. very difficult or damaged text) will not persist indefinitely without being retired.
...
For example, a subject upload for 20 subjects would need to include 20 image files and 20 corresponding plain text files, as well as a subject [manifest](https://help.zooniverse.org/transcription-project-guide/project-data/#232-manifests-and-metadata), which provides the ‘roadmap’ during the upload process for which image and text files should be paired together. The manifest can also contain additional columns containing any metadata you would like to remain associated with the subjects.
